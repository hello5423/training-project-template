...
const indexLinkFolder = JSON.parse(localStorage.getItem('indexLinkFolder') || '["D0D0AADD-B674-4434-09F0-08DB12FD2CFE"]');
const renderTableItem = item => {
  ...
  const tableItem = `
      ...
      <div class="table-section">
        ...
        <div class="w-25 item">
          <span class="nextFolder"
                title="${item.name + (item.extension ? `.${item.extension}` : '')}"
                data-name="${item.name}">${item.name + (item.extension ? `.${item.extension}` : '')}</span>
        </div>
        <div class="w-15 item">
    ...
    <span data-type="${isFile ? 'file' : 'folder'}" data-extension="${item.extension}" data-id="${item.id}" class="edit-icon">
    ...
    case undefined:
      {
        ...
        $FOLDER_ITEM.find('#delete-icon').on('click', e => {
          ...
          _services_folder__WEBPACK_IMPORTED_MODULE_0__["default"].deleteById(item.id).then(() => {
            $FOLDER_ITEM.remove();
          });
        });
    ...
    default:
      {
        ...
        $FILE_ITEM.find('#delete-icon').on('click', e => {
          $(e.currentTarget).replaceWith('<div class="spinner-border spinner-border-sm" role="status"><span class="sr-only">Loading...</span></div>');
          _services_folder__WEBPACK_IMPORTED_MODULE_0__["default"].removeFile(item.id).then(() => {
            $FILE_ITEM.remove();
          });
        });
        return $FILE_ITEM;
...
/* harmony import */ var _components_modal__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ../components/_modal */ "./src/scripts/components/_modal.ts");
/* harmony import */ var _components_table__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! ../components/_table */ "./src/scripts/components/_table.ts");
/* harmony import */ var _utilities_helper__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(/*! ../utilities/_helper */ "./src/scripts/utilities/_helper.ts");
...
const indexLinkFolder = JSON.parse(localStorage.getItem('indexLinkFolder') || '["D0D0AADD-B674-4434-09F0-08DB12FD2CFE"]');
const indexFolder = indexLinkFolder[indexLinkFolder.length - 1] || 'D0D0AADD-B674-4434-09F0-08DB12FD2CFE';
Object(_utilities_helper__WEBPACK_IMPORTED_MODULE_6__["default"])(() => {
  Object(_components_grid__WEBPACK_IMPORTED_MODULE_3__["default"])();
});
const renderTable = async () => {
  _services_folder__WEBPACK_IMPORTED_MODULE_1__["default"].getById(indexFolder).then(folder => {
    ...
    $('#container-header-title').html(folder ? '' : 'Folder not found!');
    if (!folder) return;
    // in case user clicks to folder
    $('#backIcon').on('click', () => {
      indexLinkFolder.pop();
      localStorage.setItem('indexLinkFolder', Object(_utilities_helper__WEBPACK_IMPORTED_MODULE_6__["convertToJSONString"])(indexLinkFolder));
      window.location.reload();
    });
    $('#container-header-title').text(folder.name);
    folder.subFolders.forEach(item => {
      const $FOLDER_ITEM = _components_table__WEBPACK_IMPORTED_MODULE_5__["default"].renderTableItem(item);
      $('.table-container').append($FOLDER_ITEM);
    });
    folder.files.forEach(item => {
      const $FILE_ITEM = _components_table__WEBPACK_IMPORTED_MODULE_5__["default"].renderTableItem(item);
      $('.table-container').append($FILE_ITEM);
    });
  });
};
$(document).ready(() => {
  _services_user__WEBPACK_IMPORTED_MODULE_2__["default"].getUserInfo().then(user => {
    ...
    $('#file-input').change(e => {
      ...
      _services_folder__WEBPACK_IMPORTED_MODULE_1__["default"].addFile(indexFolder, {
        name,
        extension,
        modifiedBy: 'admin'
      }).then(res => {
        if (!res) return;
        $('.table-container').append(_components_table__WEBPACK_IMPORTED_MODULE_5__["default"].renderTableItem(res));
      });
    });
    ...
    $('#btnNewItem').on('click', () => {
      ...
      _components_modal__WEBPACK_IMPORTED_MODULE_4__["default"].setValueForCommonModal('Add', 'folder', '', '', '');
    });
    $('#logoutBtn').click(() => {
      window.location.href = 'https://localhost:44307/api/Auth/signout';
    });
    // add new item
    $('#btnSubmit').on('click', () => {
      const type = $('#type').val();
      ...
      switch (btnType) {
        case 'add':
          {
            if (type === 'file') {
              _components_modal__WEBPACK_IMPORTED_MODULE_4__["default"].setLoadingForSubmitButton(true);
              _services_folder__WEBPACK_IMPORTED_MODULE_1__["default"].addFile(indexFolder, {
                name,
                extension,
                modifiedBy: 'admin'
                ...
                if (!res) return;
                _components_modal__WEBPACK_IMPORTED_MODULE_4__["default"].setLoadingForSubmitButton(false);
                $('.table-container').append(_components_table__WEBPACK_IMPORTED_MODULE_5__["default"].renderTableItem(res));
                $('#commonModal').modal('hide');
              });
            } else {
              _components_modal__WEBPACK_IMPORTED_MODULE_4__["default"].setLoadingForSubmitButton(true);
              _services_folder__WEBPACK_IMPORTED_MODULE_1__["default"].create(indexFolder, {
                name,
                modifiedBy: 'admin'
              }).then(res => {
                _components_modal__WEBPACK_IMPORTED_MODULE_4__["default"].setLoadingForSubmitButton(false);
                if (!res) return;
                const $FOLDER_ITEM = _components_table__WEBPACK_IMPORTED_MODULE_5__["default"].renderTableItem(res);
        ...
        case 'edit':
          {
            ...
            if (type === 'file') {
              _components_modal__WEBPACK_IMPORTED_MODULE_4__["default"].setLoadingForSubmitButton(true);
              _services_file__WEBPACK_IMPORTED_MODULE_0__["default"].update({
                modifiedAt: new Date(),
                modifiedBy: 'admin',
                name,
                extension,
                ...
                _components_modal__WEBPACK_IMPORTED_MODULE_4__["default"].setLoadingForSubmitButton(false);
                if (!res) return;
                const $FILE_ITEM = _components_table__WEBPACK_IMPORTED_MODULE_5__["default"].renderTableItem(res);
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _config_axios__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ../config/_axios */ "./src/config/_axios.ts");

const update = async input => {
  // Wait for 1 seconds to continue doing
  const response = await _config_axios__WEBPACK_IMPORTED_MODULE_0__["default"].put(`/File/${input.id}`, {
    name: input.name,
...
const create = async (currentFolderIndex, folderItem) => {
  // Wait for 1 seconds to continue doing
  const response = await _config_axios__WEBPACK_IMPORTED_MODULE_0__["default"].post(`/Folder`, {
...
const getById = async id => {
  // Wait for 1 seconds to continue doing
  try {
    const response = await _config_axios__WEBPACK_IMPORTED_MODULE_0__["default"].get(`/Folder/${id}`, {
      withCredentials: true
    });
...
const addFile = async (id, file) => {
  ...
  const response = await _config_axios__WEBPACK_IMPORTED_MODULE_0__["default"].post(`/File`, {
    name: file.name,
...
const removeFile = async idFile => {
  // wait for 1 seconds to continue doing
  const response = await _config_axios__WEBPACK_IMPORTED_MODULE_0__["default"].delete(`/File/${idFile}`);
  const file = response.data;
  if (!file) {
    return null;
  }
...
const update = async (id, input) => {
  // Wait for 1 seconds to continue doing
  const response = await _config_axios__WEBPACK_IMPORTED_MODULE_0__["default"].put(`/Folder/${id}`, {
    name: input.name,
    modifiedBy: input.modifiedBy || ''
  });
  const folder = response.data;
...
const deleteById = async folderRemoveId => {
  // Wait for 1 seconds to continue doing
  const response = await _config_axios__WEBPACK_IMPORTED_MODULE_0__["default"].delete(`/Folder/${folderRemoveId}`);
  const folder = response.data;
  return folder;
};
const folderService = {
...
const getUserInfo = async () => {
  const response = await _config_axios__WEBPACK_IMPORTED_MODULE_0__["default"].get('Auth/me');
  return response.data;
};
const userService = {
  getUserInfo
