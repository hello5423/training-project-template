...
const renderTableItem = item => {
  ...
  <div class="table-section">
    <div class="w-5 item text-end">
      <i class="fa-solid fa-${Object.hasOwnProperty.call(item, 'extension') ? 'file' : 'folder'}"></i>
    </div>
    <div class="w-35 item" id="name">
      <span>${item.name}</span>
    </div>
    <div class="w-20 item">
      <span>${Object(_utilities_helper__WEBPACK_IMPORTED_MODULE_3__["convertToDateFormat"])(item.modifiedAt)}</span>
    </div>
    <div class="w-20 item">
...
jquery__WEBPACK_IMPORTED_MODULE_0___default()(() => {
  const indexFolder = parseInt(localStorage.getItem('index') || '1', 10);
  // Render item list after finish loading DOM
  jquery__WEBPACK_IMPORTED_MODULE_0___default()('.table-container').html('Loading...');
  _services_folder__WEBPACK_IMPORTED_MODULE_1__["default"].getById(indexFolder).then(folder => {
    jquery__WEBPACK_IMPORTED_MODULE_0___default()('.table-container').html(folder !== undefined ? '' : 'Folder not found!');
    if (folder === undefined) return;
    folder.subFolders.forEach(item => {
      const $FOLDER_ITEM = jquery__WEBPACK_IMPORTED_MODULE_0___default()(renderTableItem(item));
      $FOLDER_ITEM.find('#delete-icon').on('click', () => {
        _services_folder__WEBPACK_IMPORTED_MODULE_1__["default"].deleteById(folder.id).then(() => {
          $FOLDER_ITEM.remove();
        });
      });
      ...
      jquery__WEBPACK_IMPORTED_MODULE_0___default()('.table-container').append($FOLDER_ITEM);
    });
    folder.files.forEach(item => {
      const $FILE_ITEM = jquery__WEBPACK_IMPORTED_MODULE_0___default()(renderTableItem(item));
      ...
      jquery__WEBPACK_IMPORTED_MODULE_0___default()('.table-container').append($FILE_ITEM);
    });
  });
  // Click New Item button
  jquery__WEBPACK_IMPORTED_MODULE_0___default()('#new-item').on('click', () => {
    const promptQuery = prompt('Enter type of new item (folder or file)');
    ...
    if (promptQuery === 'file') {
      ...
      _services_folder__WEBPACK_IMPORTED_MODULE_1__["default"].addFile(indexFolder, fileTest).then(res => {
        if (!res) return;
        jquery__WEBPACK_IMPORTED_MODULE_0___default()('.table-container').append(renderTableItem(fileTest));
      });
      ...
      _services_folder__WEBPACK_IMPORTED_MODULE_1__["default"].create(indexFolder, folderTest).then(res => {
        if (!res) return;
        const $FOLDER_ITEM = jquery__WEBPACK_IMPORTED_MODULE_0___default()(renderTableItem(folderTest));
        $FOLDER_ITEM.find('#delete-icon').on('click', () => {
          _services_folder__WEBPACK_IMPORTED_MODULE_1__["default"].deleteById(folderTest.id).then(() => {
            $FOLDER_ITEM.remove();
          });
        });
        $FOLDER_ITEM.find('#name').on('click', () => {
        ...
        jquery__WEBPACK_IMPORTED_MODULE_0___default()('.table-container').prepend($FOLDER_ITEM);
      });
    } else {
